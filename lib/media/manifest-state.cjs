...
const QUOTA_RECORD_KEY = "media-quota";
const QUOTA_ATTACH_FRESH_MS = 15 * 60 * 1000;

function quotaLimit() {
  return Math.max(1, Number(process.env.BSD_DAILY_QUOTA || 7500));
}

let localQuotaRecord = null;

async function readQuotaRecord() {
  const today = new Date().toISOString().slice(0, 10);
  let record = null;
  if (databaseConfigured()) {
    try { record = (await readRuntimeSetting(QUOTA_RECORD_KEY))?.value || null; } catch { record = null; }
  } else {
...
async function writeQuotaRecord(record) {
  if (databaseConfigured()) {
    try { await writeRuntimeSetting(QUOTA_RECORD_KEY, record); } catch {}
  } else {
...
async function noteQuotaExhausted() {
  const record = await readQuotaRecord();
  record.rateLimitedUntil = nextQuotaResetIso();
  await writeQuotaRecord(record);
  return record;
}
...
async function persistBsdCaches() {
  const dump = dumpBsdCaches();
  if (!dump) return;
  if (databaseConfigured()) {
    try {
      const existing = (await readRuntimeSetting(BSD_CACHE_KEY))?.value || null;
      await writeRuntimeSetting(BSD_CACHE_KEY, mergeBsdCacheDumps(existing, dump));
    } catch {}
  } else {
    localBsdCacheBlob = mergeBsdCacheDumps(localBsdCacheBlob, dump);
  }
}
...
async function abortQuotaExhausted(id, cp) {
  if (cp) await persistStepCheckpoint(cp);
...
async function resumeDirectStep(rawLeagueId, options = {}) {
  ...
  if (isQuotaExhaustedRecord(quotaRecord)) {
    return abortQuotaExhausted(id, null);
  }
  await restorePersistedBsdCaches();
  ...
  if (reset) {
    const existing = await readStepCheckpoint(id);
  ...
  if (cp.phase === "teams") {
    ...
    if (!cp.catalog.bsdDirectoryError) {
      while (cursor < clubs.length && Date.now() < deadline && !bsdCircuitOpen()) {
        ...
        cp.done = cursor;
        if (bsdCircuitOpen()) {
          return abortQuotaExhausted(id, cp);
        }
