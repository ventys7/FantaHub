...
// Estremal ratio per gli unresolved: chiede un ritratto keyless (Wikimedia,
...
async function faceBridgeUnresolved(manifest, previousPlayers, id) {
  if (!FACE_BRIDGE_ENABLED || !manifest?.players) return;
  const nowIso = isoNow();
  ...
  // Il budget evita raffiche lente (e rate-limit) in un singolo refresh:
  // i rimanenti verranno presi al refresh successivo.
  const targets = candidateEntries.slice(0, Math.max(1, FACE_BRIDGE_MAX_LOOKUPS));
  let found = 0;
  await mapWithConcurrency(targets, 2, async ({ key, entry }) => {
    try {
...
async function buildDirectState(rawLeagueId, options = {}) {
  ...
  await mapWithConcurrency(pendingNameSearches, 3, async ({ key, asset }) => {
