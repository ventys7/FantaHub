...
function bsdCacheTtlMs() {
  return boundedEnvInteger("BSD_CACHE_TTL_MS", 604800000, 60000, 2592000000);
}

const BSD_SEARCH_MISS_TTL_MS = 24 * 3600 * 1000;

let bsdCacheRevision = 0;
let bsdCacheAcknowledgedRevision = 0;

function markBsdCachesChanged() {
  bsdCacheRevision += 1;
}

function cacheEntryFresh(entry, ttlMs, now = Date.now()) {
  return Boolean(
    entry
    && typeof entry === "object"
    && !Array.isArray(entry)
...
function cacheEntryTtl(section, entry) {
  return section === "searches" && Array.isArray(entry?.value) && entry.value.length === 0
    ? BSD_SEARCH_MISS_TTL_MS
    : bsdCacheTtlMs();
}

function pruneBsdCacheBlob(blob, now = Date.now()) {
  const out = { transfers: {}, searches: {}, seasons: {} };
  for (const section of Object.keys(out)) {
    const entries = blob?.[section];
    if (!entries || typeof entries !== "object" || Array.isArray(entries)) continue;
    for (const [key, entry] of Object.entries(entries)) {
      const valueValid = section === "searches"
        ? Array.isArray(entry?.value)
        : entry?.value && typeof entry.value === "object" && !Array.isArray(entry.value);
      if (valueValid && cacheEntryFresh(entry, cacheEntryTtl(section, entry), now)) {
        out[section][key] = { value: clone(entry.value), storedAt: entry.storedAt };
      }
    }
  }
  return out;
...
function dumpBsdCaches() {
  const maps = { transfers: transferDetailCache, searches: searchCache, seasons: seasonCache };
  const value = pruneBsdCacheBlob(Object.fromEntries(
    Object.entries(maps).map(([section, map]) => [section, Object.fromEntries(map)])
  ));
  for (const [section, map] of Object.entries(maps)) {
    for (const key of map.keys()) {
      if (!Object.hasOwn(value[section], key)) {
        map.delete(key);
        markBsdCachesChanged();
      }
    }
  }
  if (bsdCacheRevision === bsdCacheAcknowledgedRevision) return null;
  ...
  return { revision: bsdCacheRevision, value };
}

function acknowledgeBsdCaches(revision) {
  if (!Number.isFinite(revision) || revision > bsdCacheRevision) return;
  bsdCacheAcknowledgedRevision = Math.max(bsdCacheAcknowledgedRevision, revision);
}

function restoreBsdCaches(blob) {
  const now = Date.now();
  const restored = pruneBsdCacheBlob(blob, now);
  const maps = { transfers: transferDetailCache, searches: searchCache, seasons: seasonCache };
  for (const [section, map] of Object.entries(maps)) {
    const persisted = blob?.[section];
    if (persisted && typeof persisted === "object" && !Array.isArray(persisted)) {
      for (const key of Object.keys(persisted)) {
        if (!Object.hasOwn(restored[section], key)) markBsdCachesChanged();
      }
    }
    for (const [key, entry] of map) {
      if (!cacheEntryFresh(entry, cacheEntryTtl(section, entry), now)) {
        map.delete(key);
        markBsdCachesChanged();
      }
    }
    for (const [key, entry] of Object.entries(restored[section])) {
      const current = map.get(key);
      if (!current || entry.storedAt >= current.storedAt) {
        map.set(key, entry);
      }
    }
  }
...
function freshCacheEntry(map, key, ttlMs) {
  const entry = map.get(key);
  if (!entry || cacheEntryFresh(entry, ttlMs)) return entry || null;
  map.delete(key);
  markBsdCachesChanged();
  return null;
}
...
async function commitBsdCallCount(persist) {
  const count = drainBsdCallCount();
  if (!count) return 0;
  try {
    await persist(count);
    return count;
...
function clearSeasonCache() { if (seasonCache.size) { seasonCache.clear(); markBsdCachesChanged(); } }
function clearTransferCache() { if (transferDetailCache.size) { transferDetailCache.clear(); markBsdCachesChanged(); } }
...
async function resolveProviderSeason(rawLeagueId) {
  ...
  const cachedSeason = freshCacheEntry(seasonCache, league, bsdCacheTtlMs());
  if (cachedSeason) return cachedSeason.value;
  let result = null;
  try {
...
async function fetchPlayerTransferDetail(playerId) {
  ...
  if (!/^\d+$/.test(id)) return null;
  const cached = freshCacheEntry(transferDetailCache, id, bsdCacheTtlMs());
...
function clearSearchCache() { if (searchCache.size) { searchCache.clear(); markBsdCachesChanged(); } }
...
async function searchPlayerByName(rawName, country = "", options = {}) {
  ...
  const existingSearch = searchCache.get(cacheKey);
  const cachedSearch = freshCacheEntry(searchCache, cacheKey, cacheEntryTtl("searches", existingSearch));
  if (cachedSearch) {
