...
async function searchPlayerByName(rawName, country = "", options = {}) {
  ...
  let hits = [];
  try {
    const payload = await bsdGet("/api/players/", { search: query, limit: Math.max(1, Number(options.limit || 50)) });
    hits = collectSearchPlayers(payload)
      .filter((candidate) => !country || !candidate.country || normalize(candidate.country) === normalize(country));
  } catch (error) {
    ...
    // cacheato come "0 risultati": un cache di fallimento per 6h congelerebbe
