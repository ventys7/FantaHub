...
const FACE_BRIDGE_RETRY_MS = 6 * 60 * 60 * 1000;
...
async function timedFetch(url, options, fetchRequest) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);
  try {
    return await fetchRequest(url, { ...options, signal: controller.signal });
  } finally {
    clearTimeout(timer);
  }
}
...
async function wikiGet(params, runtime) {
  const url = new URL(WIKI_API);
  Object.entries(params).forEach(([key, value]) => url.searchParams.set(key, String(value)));
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    const wait = WIKI_MIN_INTERVAL_MS - (runtime.now() - runtime.lastWikiCallAt);
    if (wait > 0) await runtime.pause(wait);
    runtime.lastWikiCallAt = runtime.now();
    try {
      const response = await timedFetch(url, { headers: { "User-Agent": USER_AGENT } }, runtime.fetch);
      ...
      await runtime.pause(error?.retryAfterMs || attempt * attempt * 2000);
    }
  }
  return { query: { pages: {} } };
}
...
async function wikiSearchPages(name, runtime) {
...
async function wikiExact(name, runtime) {
  const pages = await wikiSearchPages(name, runtime);
...
async function wikiLookup(name, runtime) {
  const exact = await wikiExact(name, runtime);
  if (exact) return exact;
  return wikiSurname(name, runtime);
}

// --- TheSportsDB -----------------------------------------------------------------

async function tsdGet(params, runtime) {
  const url = new URL(TSD_API);
  Object.entries(params).forEach(([key, value]) => url.searchParams.set(key, String(value)));
  const wait = TSD_MIN_INTERVAL_MS - (runtime.now() - runtime.lastTsdCallAt);
  if (wait > 0) await runtime.pause(wait);
  runtime.lastTsdCallAt = runtime.now();
  const response = await timedFetch(url, { headers: { "User-Agent": USER_AGENT } }, runtime.fetch);
...
async function tsdSearch(query, teamName, runtime) {
  const payload = await tsdGet({ p: query.replace(/\s+/g, "_") }, runtime);
...
async function tsdLookup(name, teamName, runtime) {
  const full = await tsdSearch(name, teamName, runtime);
  if (full) return full;
  const tokens = name.split(" ").filter(Boolean);
  if (tokens.length > 1) {
    const surname = tokens.at(-1);
    if (surname.length >= 4) return tsdSearch(surname, teamName, runtime);
  }
  return null;
}
...
function createFaceBridge(dependencies = {}) {
  const cache = new Map();
  const runtime = {
    fetch: dependencies.fetch || ((...args) => globalThis.fetch(...args)),
    now: dependencies.now || Date.now,
    pause: dependencies.pause || pause,
    lastWikiCallAt: 0,
    lastTsdCallAt: 0
  };
  ...
  function flushCache() {
    cache.clear();
    runtime.lastWikiCallAt = 0;
    runtime.lastTsdCallAt = 0;
  }
  ...
  async function lookupFace(rawName, teamName = "") {
    ...
    if (cached && (cached.value !== null || runtime.now() - cached.checkedAt < FACE_BRIDGE_RETRY_MS)) {
      return cached.value;
    }

    let result = null;
    try {
      result = await wikiLookup(name, runtime);
    } catch (error) {
      // Wikipedia giu' o in rate limit: si prosegue con TheSportsDB.
    }
    if (!result) {
      try {
        result = await tsdLookup(name, teamName, runtime);
      } catch (error) {
        result = null;
      }
    }
    cache.set(key, { value: result || null, checkedAt: runtime.now() });
    return result;
  }
  ...
  return {
    ...
    flushCache,
    lookupFace,
    wikiListing: (name) => wikiSearchPages(name, runtime)
  };
}

const sharedBridge = createFaceBridge();

module.exports = {
  FACE_BRIDGE_RETRY_MS,
  absoluteThumb,
  cacheSize: sharedBridge.cacheSize,
  createFaceBridge,
  flushCache: sharedBridge.flushCache,
  lookupFace: sharedBridge.lookupFace,
  normalizeName,
  wikiListing: sharedBridge.wikiListing
};
