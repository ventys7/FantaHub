...
function cleanUrl(value, { allowEmpty = true } = {}) {
  ...
  if (parsed.protocol !== "https:" || parsed.username || parsed.password) throw new Error("URL non valido");
...
function normalizeLeagueSettings(raw = {}) {
  return {
    ...
    regolamentoDocUrl: cleanUrl(raw.regolamentoDocUrl || ""),
    calendarioDocUrl: cleanUrl(raw.calendarioDocUrl || "")
  };
}
...
async function saveLeagueSettings(rawLeagueId, rawLeagueSettings) {
  ...
  const league = normalizeLeagueSettings(rawLeagueSettings || {});
  if (databaseConfigured()) {
    const saved = await updateLeagueSettings(id, league);
    return { document: normalizeSettings(saved.value), source: "neon" };
  }
  const current = await readSettings();
  const next = structuredClone(current);
  next.leagues[id] = league;
